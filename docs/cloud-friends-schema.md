...
## 拜訪分享(帶自己的寵物、拿食物/玩具給對方的寵物吃/玩)

> ⚠ **這不是「送禮物」**(v14 澄清):挑的那樣東西是**當場給朋友的寵物吃掉 / 陪牠玩**,
> 不會進到對方的背包(`inv`),也不會從自己的背包扣掉,更不會影響任何一邊的成長值/點數。
> `visitLog` 是一本「誰來過、做了什麼」的留言簿,不是收禮匣。UI 文案(`app/visit.js` 的
> 「給牠吃 / 陪牠玩」、`app/room.js` 通知橫幅的「給小白吃了…」)與寵物的回憶台詞都照這個語意寫,
> 不要再出現「送 / 收禮物」的說法。
...
- `visitFriend` 成功後,`app/visit.js` 讓訪客帶著自己的寵物一起走進對方房間(純本機畫面,重用
  ...
  給主人的寵物——**一次拜訪限一次**,沿用既有「每位朋友每天一次」的拜訪鎖,不用另外做次數鎖。
- **朋友的房間畫牠自己的場景**(v14):`app/visit.js` 跟 `app/room.js` 一樣查 `window.PLS_SCENE_ROOM[species]`,
  朋友養的物種有專屬場景就畫牠的(大象/聖誕貓/小雞/貓頭鷹/長頸鹿…),沒有的才退回該物種主題色的通用壁紙。
  新增物種若做了 `SCENE_ROOM`,拜訪畫面自動跟著有,不用另外接。
- **挑東西的介面跟自己房間的背包托盤同一套**(v14):下緣兩個入口「給牠吃 / 陪牠玩」→ 覆蓋式托盤
  (7×2 格、數量徽章、金色食物金框、空背包提示),不再是只放得下 8 格、而且玩具永遠被食物擠掉的那一排。
- **給完會演出來**(v14):朋友的寵物走到定點,把食物吃掉(三口)/ 玩起玩具,再開心道謝 ——
  「當場給牠吃」如果沒有吃的動作就不成立。純本機演出,不寫任何 Firestore。
- 金色食物在 `visitLog` 只存 `type`/`key`/`label` 三個欄位(`firestore.rules` 的 `gift.keys().hasOnly()`),
  所以「金色」是寫進 `label`(例:`"金色的壽司"`),不另外加欄位、不用改規則。
...
- 主人這端沒有即時通知(沒有 push,純前端單機 App 沒有背景常駐),而是在小朋友下次打開房間(`app/room.js`
  ...
  (`pls.cloud.<slot>.lastVisitLogCheckAt`)的紀錄,疊一張可點掉的橫幅通知「🐾 OO家的貓頭鷹 來過我們家! /
  給小白吃了『XX』!」(v14:橫幅左邊畫來訪的那隻寵物、右邊畫那樣東西,右緣讓 160px 給積分 HUD)。
  點掉時只更新本機游標,**不寫回 Firestore**——`visitLog` 的 entry 本身永遠不會被 update/delete,
  「已讀」純粹是本機狀態。
- **真正的回饋是寵物自己講出來**(v14):同一批紀錄會寫成寵物的 `giftGot` 回憶(帶 `pet` 物種名與
  `act` = `eat`/`play`,見 `docs/export-import-schema.md` v13.1),之後房間閒聊就會冒出
  「小宇家的哈哈給我吃鯛魚燒!」。橫幅是「這次進來就看到」,回憶是「牠一直記得」。
...
- **來的是好友時,主人的寵物會講出那位朋友的近況**(v14,`room.js` 的 `friendNewsLines()` + `config.js`
  的 `talkCare.friendNews`):「聽說阿翔破到第 9 關了!」「小米家有 12 個金幣!」「牠已經是大寶了耶!」——
  素材**全部**取自這位好友的 `status` 快照(`trophy` / `trophyEn` / `points` / `dex.foods` / `stage` /
  `collection`),快照裡沒有的欄位就不生成那一句。**寧可少講,也不要讓寵物講出沒發生過的事**。
  每位客人最多講兩句、間隔開來,免得洗掉一般閒聊。
...
### v7(2026-09,拜訪體驗大修 — 「給朋友的寵物吃/玩」的語意、朋友家的場景、朋友的近況)
- **Firestore 沒有任何欄位/規則變更**:這一版全部是前端的呈現與語意修正,`players` / `friendCodes` /
  `friends` / `visits` / `visitLog` / `backups` 的結構與 `firestore.rules` 都不動。
- **語意澄清**:拜訪時給的食物/玩具是「當場給朋友的寵物吃掉 / 陪牠玩」,不是送禮物(見上方「拜訪分享」
  一節的警告框)。相關文案、寵物回憶台詞、通知橫幅全部改寫;`docs/export-import-schema.md` 記為 v13.1
  (`memo` 事件多兩個選填欄位 `pet` / `act`,schema 版本不變)。
- **`app/visit.js`**:朋友的房間改吃 `PLS_SCENE_ROOM[friend.species]`;挑東西改成跟自己房間一樣的
  背包托盤(食物/玩具兩個入口,7×2 格 + 數量徽章 + 金色食物金框,不再是 8 格上限、玩具永遠被食物擠掉);
  給完會演出「走過來吃/玩 + 道謝」;身份卡加朋友寵物的頭像與成長階段;獎盃改掛在房間牆上(原本疊在
  畫面右上角會被「看圖鑑」鈕蓋住);自己的寵物固定站左側、朋友的寵物只在中右側漫遊(原本會疊在一起)。
- **`index.html` 好友清單**:每一列畫出朋友那隻寵物的縮圖(`PLS_ACTOR.drawAt` 進小 canvas)+ 成長階段 +
  兩科獎盃 + 珍藏數的小標籤,不再只有一行暱稱。
- **`app/room.js`**:通知橫幅改成「🐾 OO家的XX 來過我們家 / 給小白吃了『YY』」並畫出寵物與道具;
  好友來作客時主人的寵物會講朋友的近況(`friendNewsLines()` + `talkCare.friendNews`,素材全部來自 `status`
  快照,沒有的欄位就不生成)。
- **順手修掉的相鄰 bug**:`app/screens.js` 的 `FOOD_NAMES` 少了二上 10 種新食物 → 許願泡泡、寵物記憶台詞、
  拿去朋友家分享的 `gift.label` 全都會顯示成「寶物」。已補齊。
