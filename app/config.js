...
window.PLS_CONFIG = {
  ...
  // v15:英文二上(english2,qa 玩法)每關至少幾題是「聽發音描寫」(單字或整句都可能抽到),
  // 其餘才是選擇題;count=10 時就是 4 題描寫 + 6 題選擇。
  qaTraceMin: 4,
  ...
  talk: {
    ...
    // 沒過關:這一次不算掉每日額度,小朋友可以馬上再挑戰,所以不要說「明天」
    almost: ['好可惜,差一點點!', '再練習一下,再挑戰一次!'],
    // v15:過關了但這一關的獎勵次數已拿滿(沒有食物/點數)
    cappedPass: ['過關了!這關獎勵拿滿了~', '好厲害!去別關賺食物吧!'],
    // v4 電子雞化:收穫(過關食物進背包)
  ...
  talkCare: {
    ...
    // v16:餵食 / 陪玩已經不加成長值時(今天的 care 額度用完;或今天成長值到頂)吃完/玩完說的話 ——
    // 把主人推去解題。v15 加了「有點飽/有點累」開頭,呼應「額度用完了」的感覺。
    eatNoGrow: ['我有點飽了,想長大要去解題喔~', '好吃!想長大就去解題吧~', '吃飽飽~陪我解題我長更快!', '謝謝主人!等一下去解題好不好?'],
    playNoGrow: ['我有點累了,想長大要去解題喔~', '好好玩!解題我會長大喔~', '玩夠了~我們去解題吧!', '謝謝主人!過一關我就長大!'],
    // 成長值到頂後吃出幸運星:只慶祝,不說 +1(實際不會再加)
    starCapped: ['亮晶晶!是幸運星耶!', '哇!吃出一顆小星星!'],
    // v16:今天還沒解題的撒嬌(解題才是長大的主要來源,寵物會主動邀)
    solveNag: ['今天還沒解題耶,陪我長大好不好?', '去數學餐廳過一關,我就長大!', '解一關我就長大一點點~',
      '英文遊戲間過關我也會長大喔!', '我好想長大,陪我解題~'],
    // v5:今天還沒被照顧的撒嬌(不催促、不負面)
    hungryNag: ['我肚子有點餓了~', '今天還沒吃東西呢…', '好想吃點心喔~'],
    playNag: ['陪我玩一下好不好?', '玩具箱在等我們喔~'],
    // v5:許願
    wish: ['我今天好想吃'],                        // + 食物名(泡泡裡直接畫食物圖)
    wishGranted: ['就是這個!是我許願的!好幸福~', '願望成真了!謝謝主人!超級好吃!'],
  ...
  talkEng: {
    ...
    // v15:英文也會沒過關(錯太多題)/ 過關但玩具拿滿
    almost: ['差一點點,再試一次!', '再聽仔細一點,你可以的!'],
    cappedPass: ['過關了!這關玩具拿滿了~', '好厲害!去別關拿玩具吧!']
  }
};
