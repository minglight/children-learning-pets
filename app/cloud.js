...
(function () {
  ...
  var flushTimer = null;
  var dirty = {};       // petId -> true(尚未上傳的變更)
  var lastPush = {};    // petId -> timestamp(節流用)

  // ── 本機雲端連結資訊(獨立於 pet schema,不進 SCHEMA_VERSION/匯出檔)──
  function localKey(petId) { return LOCAL_PREFIX + petId; }
  function loadLocal(petId) {
    try {
      var raw = localStorage.getItem(localKey(petId));
      return raw ? JSON.parse(raw) : {};
    } catch (e) { return {}; }
  }
  function saveLocal(petId, rec) {
    try { localStorage.setItem(localKey(petId), JSON.stringify(rec)); } catch (e) {}
  }
  ...
  function statusSnapshot(petData) {
    return {
      ...
      hwEarned: petData.hwEarned || 0,
      home: petData.home || { foods: [], toys: [] },
  ...
  // ── 建立/取得這隻寵物的雲端身份(players 文件 + 好友代碼 + 還原碼)──
  function ensureLinked(petId) {
    return init().then(function (ok) {
      if (!ok) return null;
      var rec = loadLocal(petId);
      if (rec && rec.playerId) return rec;
      return createPlayer(petId, rec);
    }).catch(function () { return null; });
  }

  function createPlayer(petId, rec) {
    rec = rec || {};
    var petData = ST.load(petId);
    var childNickname = petData && petData.childNickname ? String(petData.childNickname).slice(0, 6) : '';
    var petName = petData && petData.name ? String(petData.name).slice(0, 6) : '';
  ...
  function flushAll() {
    Object.keys(dirty).forEach(function (petId) {
      delete dirty[petId];
      flushPet(petId).catch(function () { dirty[petId] = true; });   // 失敗留到下次 tick 重試
    });
  }

  function flushPet(petId) {
    return ensureLinked(petId).then(function (rec) {
      if (!rec) return;
      var petData = ST.load(petId);
  ...
  function pushStatus(petId) {
    var now = Date.now();
    if (lastPush[petId] && now - lastPush[petId] < PUSH_THROTTLE_MS) return Promise.resolve();
    lastPush[petId] = now;
    return flushPet(petId).catch(function () {});
  }

  // ── 還原碼救回進度(家長區專用,不在好友面板出現)──
  // 兩步驟確認:先查這組還原碼實際是「哪個小朋友、哪種寵物」的備份,家長確認過內容後,
  // 才選要蓋掉本機哪一格(本機仍是兩個固定物種格,但不再是家長憑印象瞎猜)。
    ...
    return init().then(function (ok) {
      ...
      return db.collection('backups').doc(restoreCode).get().then(function (snap) {
        ...
        return db.collection('players').doc(data.playerId).get().then(function (pSnap) {
          ...
          return { ok: true, childNickname: p.childNickname || '', petKind: p.petKind || '' };
        });
      });
    }).catch(function () { return { ok: false, reason: 'error' }; });
  }

  function restoreFromCode(petId, restoreCode) {
    restoreCode = (restoreCode || '').trim().toUpperCase();
    if (!restoreCode) return Promise.resolve({ ok: false, reason: 'empty' });
    return init().then(function (ok) {
      if (!ok) return { ok: false, reason: 'offline' };
      return db.collection('backups').doc(restoreCode).get().then(function (snap) {
        ...
        var wrapper = { app: 'pls', version: data.snapshot._v || 3 };
        wrapper[petId] = data.snapshot;
        ST.importAll(JSON.stringify(wrapper));
        // 還原後不做「過戶」:本機沒有雲端連結才建立這台新裝置自己的身份,已連結就維持原樣。
        return ensureLinked(petId).then(function () { return { ok: true }; });
      });
    }).catch(function () { return { ok: false, reason: 'error' }; });
  }
    ...
    return init().then(function (ok) {
      ...
      return db.collection('friendCodes').doc(code).get().then(function (snap) {
        ...
        return db.collection('players').doc(friendId).get().then(function (pSnap) {
          ...
          var p = pSnap.data();
          return { ok: true, playerId: friendId, childNickname: p.childNickname || '', petKind: p.petKind };
        });
      });
    }).catch(function () { return { ok: false, reason: 'error' }; });
  }

  function addFriendByCode(petId, code) {
  ...
  function canVisitToday(petId, friendId) {
    return ensureLinked(petId).then(function (rec) {
      if (!rec) return false;
  ...
  function visitFriend(petId, friendId) {
    return ensureLinked(petId).then(function (rec) {
      if (!rec) return { ok: false, reason: 'offline' };
      ...
      return visitRef.set({ at: firebase.firestore.FieldValue.serverTimestamp() }).then(function () {
        return db.collection('players').doc(friendId).get().then(function (snap) {
          ...
          return {
            ok: true,
            friend: { playerId: friendId, childNickname: p.childNickname || '', petKind: p.petKind, status: p.status || null }
          };
        });
      }).catch(function (err) {
  ...
  // ── 拜訪分享:帶自己裝備的食物/玩具分享給主人(gift = {type:'food'|'toy', key, label})──
  // 只會在對方的 visitLog 底下新增一筆,絕對碰不到對方 players 文件本身的 status/經驗值/點數。
  function shareGift(petId, friendId, gift) {
    return ensureLinked(petId).then(function (rec) {
      if (!rec) return { ok: false, reason: 'offline' };
      var petData = ST.load(petId);
  ...
  function checkVisitLog(petId) {
    return ensureLinked(petId).then(function (rec) {
      if (!rec) return [];
      var cursor = loadLocal(petId).lastVisitLogCheckAt || 0;
      return db.collection('players').doc(rec.playerId).collection('visitLog')
        .orderBy('at', 'desc').limit(10).get().then(function (qs) {
          ...
          qs.forEach(function (d) {
            ...
            if (ts > cursor) {
              out.push({
                id: d.id, fromNickname: v.fromNickname || '', fromPetKind: v.fromPetKind,
  ...
  function advanceVisitLogCursor(petId, ts) {
    var rec = loadLocal(petId);
